...
try:
    _configured_google_sync_interval = float(
        os.environ.get("GOOGLE_SYNC_INTERVAL", "2")
    )
except (TypeError, ValueError):
    _configured_google_sync_interval = 2.0
# Two seconds is the requested maximum freshness.  The environment variable
# can make this slower, but never faster: Google can return 429s when a full
# snapshot is written several times per second.
GOOGLE_SYNC_INTERVAL = max(2.0, _configured_google_sync_interval)
GOOGLE_SHEETS_SYNC_INTERVAL_SECONDS = GOOGLE_SYNC_INTERVAL
GOOGLE_SHEET_SCOPES = [
    "https://www.googleapis.com/auth/spreadsheets",
    # drive.file cannot reliably list/delete files created by an older
    # deployment or restore files from a shared folder.  Full Drive scope is
    # required for the backup/restore/deduplication contract.
...
def init_db():
    ...
    try:
        ...
        # Keep the actual uploaded source files inside the local SQLite
        # database.  This is intentionally local-only and is not included in
        # the Google Sheets snapshot; Sheets stores metadata and state.
        c.execute('''CREATE TABLE IF NOT EXISTS hosted_file_contents
                     (user_id INTEGER,
                      relative_path TEXT,
        ...
        # A deletion is recorded before the Drive object is removed.  This
        # makes delete operations crash-safe: if the host disappears after
        # SQLite/Sheets was updated, the next worker cycle still removes the
        # old Drive object instead of leaving an orphan that can later be
        # mistaken for a valid backup.
        c.execute('''CREATE TABLE IF NOT EXISTS hosted_bot_backup_deletions
...
def stop_user_file(user_id, file_name):
    ...
    try:
        ...
        c.execute(
            "UPDATE hosted_bot_backups SET desired_status = 'stopped' "
...
def start_user_file(user_id, file_name):
    ...
    try:
        ...
        c.execute(
            "UPDATE hosted_bot_backups SET desired_status = 'active' "
            "WHERE user_id = ? AND file_name = ?",
            (user_id, file_name),
        )
        conn.commit()
...
def _google_drive_backup_name(user_id, file_name):
    """Return the single stable Drive name used for one hosted file."""
    return f"{int(user_id)}_{os.path.basename(str(file_name))}"


def _google_drive_find_backup_files(user_id, file_name):
    """Find every Drive object belonging to one logical hosted file."""
    if not _google_sheets_configured():
        return []
    ...
    backup_name = _google_drive_backup_name(user_id, file_name)
    escaped_name = backup_name.replace("\\", "\\\\").replace("'", "\\'")
    query_parts = [f"name = '{escaped_name}'", "trashed = false"]
    if GOOGLE_DRIVE_FOLDER_ID:
        escaped_folder = (
            str(GOOGLE_DRIVE_FOLDER_ID)
            .replace("\\", "\\\\")
            .replace("'", "\\'")
        )
        query_parts.append(f"'{escaped_folder}' in parents")
    ...
    result = _google_api(
        ...
        params={
            ...
            "fields": (
                "files(id,name,modifiedTime,size,parents),nextPageToken"
            ),
...
def _backup_hosted_file(user_id, file_name, file_type):
    ...
    with DB_LOCK:
        ...
        try:
            ...
            file_row = connection.execute(
                "SELECT is_stopped FROM user_files "
                "WHERE user_id = ? AND file_name = ?",
                (user_id, file_name),
            ).fetchone()
    ...
    # Reuse the indexed object when possible.  If the index was lost, reuse
    # the newest exact-name object instead of creating another Drive file.
    # Older same-name objects are deleted only after the new upload succeeds.
    matching_files = []
    try:
        matching_files = _google_drive_find_backup_files(user_id, file_name)
    except Exception as exc:
        logger.warning(
            "Could not inspect existing Drive backups for %s/%s: %s",
            user_id,
            file_name,
            exc,
        )
    # A quick delete-then-reupload of the same filename must create/reuse
    # the new backup, not resurrect an object that is already queued for
    # deletion.  Otherwise the deletion worker could remove the new upload.
    with DB_LOCK:
        connection = get_db_connection()
        try:
            pending_deletion_ids = {
                str(item[0]) for item in connection.execute(
                    """SELECT drive_file_id
                       FROM hosted_bot_backup_deletions
                       WHERE user_id = ? AND file_name = ?""",
                    (user_id, file_name),
                ).fetchall()
            }
            ...
            connection.close()
    if existing_file_id in pending_deletion_ids:
        existing_file_id = ""
    matching_files = [
        item for item in matching_files
        if str(item.get("id") or "") not in pending_deletion_ids
    ]
    matching_ids = [
        str(item["id"]) for item in matching_files if item.get("id")
    ]
    if not existing_file_id and matching_ids:
        existing_file_id = matching_ids[0]

    local_path = os.path.join(get_user_folder(user_id), file_name)
    drive_file_id = _google_drive_upload(
        local_path,
        existing_file_id=existing_file_id,
        display_name=_google_drive_backup_name(user_id, file_name),
    )
    saved_file_id = drive_file_id or existing_file_id
    backup_pending = 0 if drive_file_id else 1

    # Deduplicate only after a successful upload/update.  A failed upload must
    # never destroy the last known-good recovery copy.
    if drive_file_id:
        duplicate_ids = set(matching_ids)
        if existing_file_id:
            duplicate_ids.add(existing_file_id)
        duplicate_ids.discard(drive_file_id)
        for duplicate_id in duplicate_ids:
            if _google_drive_delete(duplicate_id):
                logger.info(
                    "Removed duplicate Drive backup %s for %s/%s.",
                    duplicate_id,
                    user_id,
                    file_name,
                )
...
def _ensure_pending_hosted_file_backups(limit=5):
    """Retry pending uploads using the same deduplicating backup path."""
    ...
    completed = 0
    for user_id, file_name, file_type, _existing_file_id, _pending_flag in pending:
        ...
        if not os.path.isfile(local_path):
            continue
        _backup_hosted_file(int(user_id), safe_name, str(file_type or ""))
        with DB_LOCK:
            connection = get_db_connection()
            try:
                row = connection.execute(
                    """SELECT drive_file_id, backup_pending
                       FROM hosted_bot_backups
                       WHERE user_id = ? AND file_name = ?""",
                    (int(user_id), safe_name),
                ).fetchone()
            finally:
                connection.close()
        if row and row[0] and not int(row[1] or 0):
            completed += 1
            logger.info(
                "Drive backup retry succeeded for %s/%s.",
                user_id,
                safe_name,
            )
    return completed
...
def _remove_hosted_backup(user_id, file_name):
    """Remove the index and queue every matching Drive object for deletion."""
    drive_file_ids = set()
    with DB_LOCK:
        connection = get_db_connection()
        try:
            ...
            if row and row[0]:
                drive_file_ids.add(str(row[0]))
            connection.execute(
                "DELETE FROM hosted_bot_backups "
                "WHERE user_id = ? AND file_name = ?",
                (user_id, file_name),
            )
            for item in drive_file_ids:
                connection.execute(
                    """INSERT OR IGNORE INTO hosted_bot_backup_deletions
                       (user_id, file_name, drive_file_id, created_at)
                       VALUES (?, ?, ?, ?)""",
                    (user_id, file_name, item, datetime.now().isoformat()),
                )
            connection.commit()
        finally:
            connection.close()

    # The database may have lost its index while the Drive upload succeeded.
    # Search by the stable app-generated name and queue all matches as well.
    try:
        for item in _google_drive_find_backup_files(user_id, file_name):
        ...
        logger.warning(
            "Could not queue unindexed Drive backups for deletion %s/%s: %s",
            user_id,
            file_name,
            exc,
        )
    return next(iter(drive_file_ids), "")


def _process_pending_backup_deletions(limit=25):
    """Delete queued Drive objects; keep failed rows for the next cycle."""
    if not _google_sheets_configured():
        return 0
    with DB_LOCK:
        connection = get_db_connection()
    ...
    deleted = 0
    for row_id, drive_file_id in rows:
        if not _google_drive_delete(str(drive_file_id or "")):
            continue
        with DB_LOCK:
            connection = get_db_connection()
            try:
                connection.execute(
                    "DELETE FROM hosted_bot_backup_deletions WHERE id = ?",
                    (row_id,),
                )
                connection.commit()
            finally:
    ...
    for user_id, file_name, drive_file_id in backups:
        try:
            ...
            if os.path.isfile(destination):
                # Even when the local persistent disk survived, clean any
                # old same-name Drive copies so the next backup remains
                # one-file-per-user-file.
                try:
                    if drive_file_id:
                        for duplicate in _google_drive_find_backup_files(
                            normalized_user_id, safe_name
                        ):
                            duplicate_id = str(duplicate.get("id") or "")
                            if duplicate_id and duplicate_id != str(drive_file_id):
                                _google_drive_delete(duplicate_id)
                except Exception as exc:
                    logger.warning(
                        "Could not deduplicate existing copies for %s/%s: %s",
                        normalized_user_id,
                        safe_name,
                        exc,
                    )
            ...
            if not drive_file_id:
                ...
                try:
                    ...
                    logger.warning(
                        "Could not deduplicate restored copies for %s/%s: %s",
                        normalized_user_id,
                        safe_name,
                        exc,
                    )
...
def _restore_one_hosted_file_from_drive(user_id, file_name):
    ...
    if not drive_file_id:
        ...
        try:
            matches = _google_drive_find_backup_files(user_id, safe_name)
            if matches:
                drive_file_id = str(matches[0]["id"])
                if len(matches) > 1:
                    logger.warning(
                        "Found %s Drive backups named %s; using the newest "
                        "and removing older duplicates after restore.",
                        len(matches),
                        _google_drive_backup_name(user_id, safe_name),
                    )
        except Exception as exc:
            logger.warning(
        ...
        # Repair the local index before downloading so a later restart can
        # restore this file without relying on a Drive name search.
        try:
            ...
            logger.warning(
                "Could not remove duplicate restore copies for %s/%s: %s",
                user_id,
                safe_name,
                exc,
            )
    ...
    # The indexed ID may point at an object deleted manually in Drive.  Do
    # one exact-name lookup before declaring the file unrecoverable, then
    # repair the index with the replacement object that actually downloaded.
    try:
        matches = _google_drive_find_backup_files(user_id, safe_name)
        for match in matches:
            replacement_id = str(match.get("id") or "")
            if not replacement_id or replacement_id == drive_file_id:
                continue
            if not _google_drive_download(replacement_id, destination):
                continue
            with DB_LOCK:
                connection = get_db_connection()
                try:
                    connection.execute(
                        """UPDATE hosted_bot_backups
                           SET drive_file_id = ?, backup_pending = 0
                           WHERE user_id = ? AND file_name = ?""",
                        (replacement_id, user_id, safe_name),
                    )
                    connection.commit()
                finally:
                    connection.close()
            logger.info(
                "Restored missing hosted file using replacement Drive ID: "
                "%s/%s",
                user_id,
                safe_name,
            )
        ...
        logger.warning(
            "Replacement Drive restore lookup failed for %s/%s: %s",
            user_id,
            safe_name,
            exc,
        )
...
def _retry_missing_hosted_file_restores(limit=5):
    ...
    for user_id, file_name, drive_file_id in backups:
        ...
        if attempted >= max(1, int(limit)):
            break
        attempted += 1
        if _restore_one_hosted_file_from_drive(int(user_id), safe_name):
            restored += 1
            logger.info("Drive retry restored file %s/%s.", user_id, safe_name)
    return restored


# File-content restore is intentionally disabled. Google Sheets is used for
# durable metadata/state only; a missing local source file must be re-uploaded.
def restore_hosted_files_from_drive():
    logger.info(
        "Drive file restore is disabled; Google Sheets metadata restore remains enabled."
    )
    return {"restored": 0, "failed": 0, "skipped": 0}
...
def _restore_one_hosted_file_from_drive(user_id, file_name):
    logger.warning(
        "Drive file restore disabled for %s/%s; re-upload the local source file.",
        user_id,
        file_name,
    )
    return False
...
def google_sheets_status_text():
    ...
    lines = [
        ...
        f"📁 *Drive folder:* "
        f"{'Configured' if GOOGLE_DRIVE_FOLDER_ID else 'Service-account Drive root'}",
        f"⏱️ *Auto-sync:* every {GOOGLE_SYNC_INTERVAL:g} seconds",
...
def google_sheets_sync_worker():
    while not _google_sheets_stop_event.is_set():
        if not google_sheet_data_loaded:
            ...
            try:
                sync_succeeded = sync_to_google_sheets()
                if not sync_succeeded:
                    _google_sheets_stop_event.wait(GOOGLE_SYNC_INTERVAL)
                    continue
                deleted = _process_pending_backup_deletions(limit=100)
                if deleted:
                    sync_to_google_sheets(force=True)
...
def _safe_local_content_path(user_id, relative_path):
    """Return a safe path inside a user's local upload directory."""
    relative_path = str(relative_path or "").replace("\\", "/")
    safe_relative_path = os.path.normpath(relative_path)
    if (
        not safe_relative_path
        or safe_relative_path in (".", "..")
        or safe_relative_path.startswith(".." + os.sep)
        or os.path.isabs(safe_relative_path)
    ):
        return None
    user_folder = os.path.abspath(get_user_folder(int(user_id)))
    destination = os.path.abspath(
        os.path.join(user_folder, safe_relative_path)
    )
    if os.path.commonpath([user_folder, destination]) != user_folder:
        return None
    return destination


def snapshot_user_files_to_local_database(user_id):
    """Store every source file in a user's folder inside SQLite."""
    user_folder = os.path.abspath(get_user_folder(int(user_id)))
    ...
    try:
        for root, directories, file_names in os.walk(user_folder):
            directories[:] = [
            ...
            for name in file_names:
                ...
                if not os.path.isfile(path) or os.path.islink(path):
                    continue
                relative_path = os.path.relpath(path, user_folder)
                safe_path = _safe_local_content_path(
                    user_id, relative_path
                )
                if not safe_path:
                    logger.warning(
                        "Skipping unsafe local database path for user %s: %s",
                        user_id,
                        relative_path,
                    )
                    continue
        ...
        with DB_LOCK:
            ...
            try:
                connection.execute(
                    "DELETE FROM hosted_file_contents WHERE user_id = ?",
                    (int(user_id),),
                )
                connection.executemany(
                    """INSERT INTO hosted_file_contents
                       (user_id, relative_path, content, sha256, updated_at)
                       VALUES (?, ?, ?, ?, ?)""",
                    snapshots,
                )
                connection.commit()
                ...
                connection.close()
        logger.info(
            "Stored %s local source file(s) in SQLite for user %s.",
            len(snapshots),
            user_id,
        )
        return True
        ...
        logger.exception(
            "Could not store local source files in SQLite for user %s.",
            user_id,
        )
        return False


def restore_local_files_from_database():
    """Recreate missing runnable files from SQLite blobs only."""
    restored = failed = 0
    with DB_LOCK:
        connection = get_db_connection()
        try:
            rows = connection.execute(
                """SELECT user_id, relative_path, content, sha256
                   FROM hosted_file_contents
                   ORDER BY user_id, relative_path"""
            ).fetchall()
        finally:
            connection.close()

    for user_id, relative_path, content, expected_sha256 in rows:
        try:
            destination = _safe_local_content_path(user_id, relative_path)
            if not destination:
                failed += 1
                continue
            ...
            if hashlib.sha256(content).hexdigest() != str(expected_sha256):
                failed += 1
                logger.error(
                    "SQLite file blob checksum failed for user %s/%s.",
                    user_id,
                    relative_path,
                )
                continue
            if os.path.isfile(destination):
                ...
                if current_sha256 == str(expected_sha256):
                    continue
                logger.warning(
                    "Replacing changed local file from SQLite: %s/%s",
                    user_id,
                    relative_path,
                )
            os.makedirs(os.path.dirname(destination), exist_ok=True)
            temporary_path = f"{destination}.restore-{os.getpid()}"
            ...
            failed += 1
            logger.exception(
                "Could not recreate local file from SQLite for user %s/%s.",
                user_id,
                relative_path,
            )
    logger.info(
        "Local SQLite file recovery finished: %s restored, %s failed.",
        restored,
        failed,
    )
...
def backfill_local_file_database():
    """Capture files from older installs that predate the SQLite blob table."""
    with DB_LOCK:
        connection = get_db_connection()
        try:
            user_ids = [
    ...
    for user_id in user_ids:
        if user_id not in existing_users:
            snapshot_user_files_to_local_database(user_id)
...
def remove_user_file_db(user_id, file_name):
    with DB_LOCK:
        ...
        try:
            ...
            c.execute(
                "DELETE FROM hosted_file_contents "
                "WHERE user_id = ? AND relative_path = ?",
                (user_id, str(file_name).replace("\\", "/")),
            )
            conn.commit()
            ...
            if sheets_saved:
                deleted_backups = _process_pending_backup_deletions(limit=100)
                if deleted_backups:
                    # Remove the now-completed deletion queue rows from the
                    # remote snapshot too.
                    _persist_file_state_to_sheets(
                        f"delete cleanup {user_id}/{file_name}"
                    )
            elif drive_file_id:
                logger.warning(
                    "Drive backup deletion queued until the deleted-file "
                    "state is confirmed in Google Sheets: %s",
                    drive_file_id,
                )
            trigger_google_sheets_sync()
...
def handle_admin_start(call):
    ...
    if not os.path.isfile(file_path):
        bot.answer_callback_query(
            call.id,
            "⚠️ Local file is missing. The file record is safe in Google "
            "Sheets; please re-upload the file to start it again.",
            show_alert=True,
        )
        return
...
def start_bot_callback(call):
    try:
        ...
        if not os.path.isfile(file_path):
            bot.answer_callback_query(
                call.id,
                "⚠️ Local file is missing. Its record is safe in Google "
                "Sheets; please re-upload the file to start it.",
                show_alert=True,
            )
            check_files_callback(call)
            return
...
if __name__ == '__main__':
    ...
    # Drive file restoration is intentionally disabled. Source files are
    # recreated only from the local SQLite file-content table.
    backfill_local_file_database()
    restore_local_files_from_database()
