...
"""
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
"""
...
GOOGLE_SYNC_INTERVAL = max(
    1.0, float(os.environ.get("GOOGLE_SYNC_INTERVAL", "5"))
)
...
RUNTIME_DB_URI = "file:gxhosting_runtime?mode=memory&cache=shared"
DB_LOCK = threading.RLock()
_runtime_db_anchor = sqlite3.connect(
    RUNTIME_DB_URI, uri=True, check_same_thread=False
)
_runtime_db_anchor.execute("PRAGMA foreign_keys = ON")
_runtime_db_anchor.commit()
_sqlite_connect = sqlite3.connect


def get_db_connection():
    """Return a connection to the transient runtime database.

    Google Sheets is the only durable backend. This shared in-memory database
    exists so the existing feature code can keep using transactions safely.
    """
...
def _runtime_sqlite_connect(*_args, **_kwargs):
    """Compatibility bridge: legacy queries always hit memory, never disk."""
    return _sqlite_connect(
        RUNTIME_DB_URI, uri=True, check_same_thread=False
    )


# Existing feature handlers still issue SQL statements. Redirect their
# connection calls to the shared runtime database while Google Sheets remains
# the only durable backend.
sqlite3.connect = _runtime_sqlite_connect
DATABASE_PATH = ":runtime-only:"
IROTECH_DIR = ":runtime-only:"
...
def init_db():
    """Initialize the transient query schema; no database file is created."""
    logger.info("Initializing in-memory runtime schema.")
...
# --- Database Operations ---
# DB_LOCK and the runtime connection are initialized with the storage config
# above. There is intentionally no on-disk database to initialize here.
...
def _google_drive_upload(local_path, existing_file_id="", display_name=None):
    """Back up one hosted source file to the service account's Drive."""
    if not _google_sheets_configured() or not os.path.isfile(local_path):
...
def _backup_hosted_file(user_id, file_name, file_type):
    ...
    drive_file_id = _google_drive_upload(
        local_path, display_name=f"{user_id}_{os.path.basename(file_name)}"
    )
    with DB_LOCK:
        ...
        try:
            connection.execute(
                """INSERT OR REPLACE INTO hosted_bot_backups
                   (user_id, file_name, file_type, drive_file_id, desired_status)
                   VALUES (?, ?, ?, ?, ?)""",
                (user_id, file_name, file_type, drive_file_id, "active"),
            )
            connection.commit()
        finally:
            connection.close()
...
def restore_hosted_files_from_drive():
    ...
    for user_id, file_name, drive_file_id in backups:
        ...
        if not drive_file_id:
            failed += 1
            logger.error(
                "Drive restore has no backup ID for user %s, file %s.",
                user_id, safe_name,
            )
        elif _google_drive_download(drive_file_id, destination):
            restored += 1
            logger.info("Restored user file from Drive: %s/%s", user_id, safe_name)
        else:
            failed += 1
    logger.info(
...
def _ensure_google_sheet_tabs(token):
    ...
    def refresh_tabs(response):
        google_sheet_tab_ids = {
            item.get("properties", {}).get("title"): item.get(
                "properties", {}
            ).get("sheetId")
...
def _restore_runtime_table(table_name, rows):
    if rows is None:
        return
    with DB_LOCK:
        metadata_connection = get_db_connection()
        try:
            metadata = metadata_connection.execute(
                f'PRAGMA table_info("{table_name}")'
            ).fetchall()
        finally:
...
def _sync_to_google_sheets_unlocked(force=False):
    ...
    try:
        ...
        _google_api(
            ...
            token,
            {"ranges": [f"{name}!A:ZZ" for name in rows_by_tab]},
        )
        _google_api(
            "POST",
    ...
    try:
        ...
        for table_name in GOOGLE_SHEETS_SYNC_TABLES:
            _restore_runtime_table(
                table_name, _sheet_rows(token, table_name)
            )
...
if __name__ == '__main__':
    ...
    logger.info("📊 Durable storage: Google Sheets")
    ...
    # Never start from the empty in-memory database when a durable store is
    # expected. Doing so used to leave bots stopped after a transient Sheets
    # failure, and the later background retry loaded data without rerunning
    # Drive restore or process recovery.
    if not _google_sheets_configured():
        raise RuntimeError(
            "Google Sheets/Drive persistence is not configured. "
            "Set GOOGLE_SHEET_ID and GOOGLE_SERVICE_ACCOUNT_JSON; "
            "startup recovery is refusing to run with empty data."
        )
    while not load_google_sheet_data():
        logger.critical(
            "Durable data load failed. Keeping the bot out of polling and "
            "retrying Google Sheets in 30 seconds to avoid overwriting data."
        )
        time.sleep(30)
