...
if __name__ == '__main__':
    ...
    if _local_database_is_ready():
        ...
        if not _google_sheets_configured():
            raise RuntimeError(
                "No existing local database and Google backup is not configured. "
                "Set GOOGLE_SHEET_ID and GOOGLE_SERVICE_ACCOUNT_JSON before "
                "starting a fresh instance."
            )
        while not load_google_sheet_data():
            if google_sheet_data_loaded:
                logger.warning(
                    "Google data was restored locally; its write-back failed. "
                    "Starting with the restored data and retrying sync in background."
                )
                break
            logger.critical(
                "Google data could not be loaded. Keeping polling stopped to "
                "protect existing records; retrying in 30 seconds."
            )
            time.sleep(30)
        load_data()
