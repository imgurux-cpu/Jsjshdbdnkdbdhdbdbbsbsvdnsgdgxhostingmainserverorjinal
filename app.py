...
def restore_hosted_files_from_drive():
    ...
    for user_id, file_name, drive_file_id in backups:
        ...
        if not os.path.isfile(destination) and drive_file_id:
            _google_drive_download(drive_file_id, destination)
...
def sync_to_google_sheets(force=False):
    ...
    try:
        ...
        signature = hashlib.sha256(
            json.dumps(
                rows_by_table, ensure_ascii=False, sort_keys=True
            ).encode("utf-8")
        ).hexdigest()
        ...
        _google_api(
            ...
            token,
            {"ranges": [f"{name}!A:ZZ" for name in GOOGLE_SHEETS_SYNC_TABLES]},
        )
        _google_api(
            "POST",
...
def remove_user_file_db(user_id, file_name):
    with DB_LOCK:
        ...
        try:
            ...
            _remove_hosted_backup(user_id, file_name)
            trigger_google_sheets_sync()
