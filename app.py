...
def google_sheets_sync_worker():
    while not _google_sheets_stop_event.is_set():
        if not google_sheet_data_loaded:
            ...
            try:
                # Publish the deletion queue before removing Drive objects.
                # If the host dies after this point, the next startup still
                # knows that those IDs must never be used for restore.
                ...
                if deleted:
                    # The queue rows are part of the durable snapshot; clear
                    # them remotely after their Drive objects are gone.
                    sync_to_google_sheets(force=True)
...
def handle_admin_start(call):
    ...
    if not os.path.isfile(file_path):
        if not _restore_one_hosted_file_from_drive(target_user_id, file_name):
            bot.answer_callback_query(
                call.id,
                "⚠️ File missing and Drive restore failed.",
                show_alert=True,
            )
            return
