...
# Use only Python standard-library modules here so a fresh host can install
# Google Sheets and bot dependencies before third-party imports are loaded.
...
# File controls use these two IDs consistently everywhere.  Do not use the
# rotating/global emoji renderer for file state: a stopped file must always
# show OFF and a running file must always show ON.
EMOJI_FILE_OFF = "6053202133887490757"
EMOJI_FILE_ON = "6053128350644311646"
...
def file_status_emoji_tag(is_running=False, is_stopped=False):
    """Return the permanent file ON/OFF emoji requested by the admin."""
    emoji_id = EMOJI_FILE_OFF if is_stopped or not is_running else EMOJI_FILE_ON
    fallback = "⚫" if is_stopped or not is_running else "🟢"
    return f'<tg-emoji emoji-id="{emoji_id}">{fallback}</tg-emoji>'


def file_status_emoji_id(is_running=False, is_stopped=False):
    """Return the permanent file status emoji ID for an inline button."""
    return EMOJI_FILE_OFF if is_stopped or not is_running else EMOJI_FILE_ON
...
# --- Configuration ---
# Render must provide secrets through environment variables. Never commit
# bot tokens or Google service-account keys in source code.
...
YOUR_USERNAME = "@iamGuruX"
UPDATE_CHANNEL = os.environ.get('UPDATE_CHANNEL', 'https://t.me/GxHostingUpdate')
...
IROTECH_DIR = "runtime-only (not persisted)"
# Legacy handlers still use SQLite-shaped queries, but this database is
# shared in-memory only. Google Sheets/Drive is the only durable store.
DATABASE_PATH = "file:gx_hosting_bot_runtime?mode=memory&cache=shared"
_SQLITE_MEMORY_URI = "file:gx_hosting_bot_runtime?mode=memory&cache=shared"
_sqlite_connect = sqlite3.connect
_sqlite_keepalive = _sqlite_connect(
    _SQLITE_MEMORY_URI, uri=True, check_same_thread=False
)
DB_LOCK = threading.RLock()


def _google_only_sqlite_connect(database, *args, **kwargs):
    """Redirect the legacy SQLite handlers away from disk."""
    if database == DATABASE_PATH:
        kwargs.setdefault("uri", True)
        kwargs.setdefault("check_same_thread", False)
        return _sqlite_connect(_SQLITE_MEMORY_URI, *args, **kwargs)
    return _sqlite_connect(database, *args, **kwargs)


sqlite3.connect = _google_only_sqlite_connect

# --- Google Sheets persistence ---
# Direct source-code setup is supported. Replace the placeholders, or set the
# same values in GOOGLE_SHEET_ID and GOOGLE_SERVICE_ACCOUNT_JSON at runtime.
GOOGLE_SHEET_ID = (
...
GOOGLE_SERVICE_ACCOUNT_JSON = {
  ...
  "project_id": "sonic-diorama-509407-t6",
  "private_key_id": "befd8c0b38d061d0d046a5806ec0d46bb650a2e5",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "gxhostingbotsecondbutmain@sonic-diorama-509407-t6.iam.gserviceaccount.com",
  "client_id": "110999945736574460236",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/gxhostingbotsecondbutmain%40sonic-diorama-509407-t6.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
}
# A host environment variable may override the source-code credential. The
# direct dictionary above is also accepted by _read_service_account_info().
if os.environ.get("GOOGLE_SERVICE_ACCOUNT_JSON"):
    GOOGLE_SERVICE_ACCOUNT_JSON = os.environ["GOOGLE_SERVICE_ACCOUNT_JSON"]
# Ignore any legacy inline value in older copies. Render must supply the
# credential explicitly; an empty value intentionally disables persistence.
GOOGLE_SERVICE_ACCOUNT_JSON = (
...
# Example:
# GOOGLE_SHEET_ID = "1abc...your spreadsheet id..."
# GOOGLE_SERVICE_ACCOUNT_JSON = r'''{"type":"service_account", ...}'''
try:
    GOOGLE_SYNC_INTERVAL = max(
        1.0, float(os.environ.get("GOOGLE_SYNC_INTERVAL", "2"))
    )
except (TypeError, ValueError):
    GOOGLE_SYNC_INTERVAL = 2.0
GOOGLE_SHEET_NAMES = ["Users", "BotConfig", "HostedBots", "Summary", "RuntimeData"]
LEGACY_GOOGLE_SHEET_NAMES = ["DatabaseDump", "HostedFiles"]
GOOGLE_SHEET_HEADERS = {
    ...
    "HostedBots": [
        "owner_id", "file_name", "file_type", "desired_status", "drive_file_id"
    ],
    "Summary": ["metric", "value"],
    "RuntimeData": ["table_name", "row_index", "row_json", "schema_sql"],
}
GOOGLE_SHEET_SCOPES = [
    "https://www.googleapis.com/auth/spreadsheets",
    "https://www.googleapis.com/auth/drive.file",
]
_google_runtime_lock = threading.RLock()
_google_sheet_loaded = False
_google_last_signature = None
_google_last_error = ""
_google_auth_warning_printed = False
_google_sync_scheduled = False
...
OTP_DEPOSITE_BUTTONS = [
    ...
    [{"text": "𝙋𝘼𝙔𝙈𝙀𝙉𝙏 𝙈𝙊𝘿𝙀", "icon_custom_emoji_id": EMOJI_BINANCE, "style": "success"}],
...
ADMIN_PANEL_BUTTONS = [
    ...
    [{"text": "𝙂𝙊𝙊𝙂𝙇𝙀 𝙎𝙔𝙉𝘾 𝘾𝙃𝙀𝘾𝙆", "icon_custom_emoji_id": EMOJI_REFRESH, "style": "primary"}],
    # 🔥 Back Button Emoji পরিবর্তন
    [{"text": "𝘽𝘼𝘾𝙆 𝙏𝙊 𝘼𝘿𝙈𝙄𝙉 𝙋𝘼𝙉𝙀𝙇", "icon_custom_emoji_id": EMOJI_BACK, "style": "primary"}]
]
...
@bot.message_handler(func=lambda message: message.text == "𝙂𝙊𝙊𝙂𝙇𝙀 𝙎𝙔𝙉𝘾 𝘾𝙃𝙀𝘾𝙆")
def google_sync_check_button(message):
    """Show the current Google Sheets restore/sync state to an admin."""
    if not is_otp_admin(message.from_user.id):
        bot.reply_to(
            message,
            render_body_text("⛔ *Unauthorized!*"),
            parse_mode='HTML'
        )
        return

    # A button press should give a fresh result from the Google-only store.
    with _google_runtime_lock:
        sync_ok = sync_to_google_sheets_v2(force=True)
    status_text = (
        "📊 GOOGLE SHEETS SYNC\n\n"
        f"Status: {'READY' if sync_ok else 'ERROR'}\n"
        "Tabs: Users, BotConfig, HostedBots, Summary, RuntimeData"
    )
    if sync_ok:
        status_text += "\n\n✅ Manual sync check completed successfully."
    elif google_sheets_configured():
        status_text += "\n\n⚠️ Manual sync check failed; see the error above."
    bot.reply_to(message, render_body_text(status_text), parse_mode='HTML')
...
def google_sheets_configured():
    return bool(
        GOOGLE_SHEET_ID
        and not GOOGLE_SHEET_ID.startswith("PASTE_")
...
def _google_access_token():
    global _google_auth_warning_printed
    try:
        from google.oauth2.service_account import Credentials
        from google.auth.transport.requests import Request
    except ImportError:
        if not _google_auth_warning_printed:
            logger.error(
                "Google Sheets needs google-auth. Install it from requirements.txt."
            )
            _google_auth_warning_printed = True
        return None
...
def _ensure_google_sheet_tabs(token):
    ...
    missing = [name for name in GOOGLE_SHEET_NAMES if name not in existing]
    if missing:
        _google_api(
            "POST",
...
def _google_sheet_rows(token, sheet_name):
    url = (
        f"https://sheets.googleapis.com/v4/spreadsheets/"
        f"{GOOGLE_SHEET_ID}/values/{sheet_name}"
    )
    values = _google_api("GET", url, token).get("values", [])
    if not values:
        return []
...
def _text_chunks(value, chunk_size=42000):
    value = str(value)
    return [
...
def _runtime_sql_dump():
    with DB_LOCK:
        return "\n".join(_sqlite_keepalive.iterdump())


def _restore_runtime_sql_dump(chunks):
    if not chunks:
        return False
    sql_dump = "".join(
        str(row.get("sql_chunk", ""))
        for row in sorted(
            chunks,
    ...
    if not sql_dump.strip():
        return False
    with DB_LOCK:
        current = _sqlite_keepalive.execute(
            "SELECT name FROM sqlite_master "
            "WHERE type IN ('table', 'index', 'trigger', 'view') "
            "AND name NOT LIKE 'sqlite_%'"
        ).fetchall()
        for (name,) in current:
            _sqlite_keepalive.execute(
        ...
        _sqlite_keepalive.executescript(sql_dump)
        _sqlite_keepalive.commit()
    return True


def _hosted_file_rows():
    rows = []
    if not os.path.isdir(UPLOAD_BOTS_DIR):
        return rows
    for owner_name in os.listdir(UPLOAD_BOTS_DIR):
        owner_dir = os.path.join(UPLOAD_BOTS_DIR, owner_name)
        if not owner_name.isdigit() or not os.path.isdir(owner_dir):
            continue
        for root, dirs, files in os.walk(owner_dir):
            ...
            for file_name in files:
                if file_name.endswith(".log") or file_name.endswith(".pyc"):
                    continue
                path = os.path.join(root, file_name)
                try:
                    with open(path, "rb") as handle:
                        encoded = base64.b64encode(handle.read()).decode("ascii")
                    relative_path = os.path.relpath(path, owner_dir)
                    modified_at = str(int(os.path.getmtime(path)))
                except (OSError, ValueError) as exc:
                    logger.warning("Skipping hosted file %s: %s", path, exc)
                    continue
                for chunk_index, chunk in enumerate(_text_chunks(encoded)):
                    rows.append({
...
def _restore_hosted_files(rows):
    if not rows:
        return 0
    shutil.rmtree(UPLOAD_BOTS_DIR, ignore_errors=True)
    os.makedirs(UPLOAD_BOTS_DIR, exist_ok=True)
    grouped = {}
    for row in rows:
        try:
        ...
        if owner_id <= 0 or not relative_path:
            continue
        grouped.setdefault((owner_id, relative_path), []).append(
            (chunk_index, str(row.get("content_b64", "")))
        )

    restored = 0
    for (owner_id, relative_path), chunks in grouped.items():
        owner_dir = os.path.abspath(os.path.join(UPLOAD_BOTS_DIR, str(owner_id)))
        target = os.path.abspath(os.path.join(owner_dir, relative_path))
        if os.path.commonpath([owner_dir, target]) != owner_dir:
            logger.warning("Rejected unsafe hosted file path: %s", relative_path)
            continue
        try:
            ...
            logger.warning("Could not restore hosted file %s: %s", target, exc)
    return restored


def _google_summary_rows():
    try:
        with DB_LOCK:
            table_count = _sqlite_keepalive.execute(
        ...
        table_count = 0
    return [
        {"metric": "sqlite_tables", "value": table_count},
        {"metric": "hosted_files", "value": len(_hosted_file_rows())},
        {"metric": "storage_mode", "value": "Google Sheets + runtime memory"},
        {"metric": "startup_file_policy", "value": "restore only; never auto-start"},
    ]


def _google_rows_to_matrix(rows, sheet_name):
    headers = GOOGLE_SHEET_HEADERS[sheet_name]
    return [headers] + [
...
def sync_to_google_sheets(force=False):
    global _google_last_signature, _google_last_error
    if not google_sheets_configured():
        _google_last_error = (
            "Set GOOGLE_SHEET_ID and GOOGLE_SERVICE_ACCOUNT_JSON."
        )
        return False
    if not _google_sheet_loaded:
        _google_last_error = (
            "Google Sheets restore has not completed yet; "
            "sync is paused until startup restore succeeds."
        )
        return False
    try:
        database_rows = [
            {"chunk_index": index, "sql_chunk": chunk}
            for index, chunk in enumerate(_text_chunks(_runtime_sql_dump()))
        ]
        rows_by_sheet = {
            "DatabaseDump": database_rows,
            "HostedFiles": _hosted_file_rows(),
            "Summary": _google_summary_rows(),
        }
        ...
        if not force and signature == _google_last_signature:
            return True
        ...
        _google_api(
            ...
            token,
            {
                ...
                "data": [
                    {
                        "range": f"{name}!A1",
                        "values": _google_rows_to_matrix(rows_by_sheet[name], name),
                    }
                    for name in GOOGLE_SHEET_NAMES
                ],
            },
        )
        _google_last_signature = signature
        _google_last_error = ""
        return True
    except Exception as exc:
        _google_last_error = str(exc)
        logger.error("Google Sheets sync failed: %s", exc)
        return False


def load_google_sheet_data():
    global _google_sheet_loaded, _google_last_error
    if not google_sheets_configured():
        _google_last_error = (
            "Set GOOGLE_SHEET_ID and GOOGLE_SERVICE_ACCOUNT_JSON "
            "or replace the placeholders in the source configuration."
        )
        logger.warning("Google Sheets is not configured; no durable data loaded.")
        return False
    try:
        token = _google_access_token()
        if not token:
            return False
        _ensure_google_sheet_tabs(token)
        database_rows = _google_sheet_rows(token, "DatabaseDump")
        hosted_rows = _google_sheet_rows(token, "HostedFiles")
        if database_rows:
            _restore_runtime_sql_dump(database_rows)
        restored_files = _restore_hosted_files(hosted_rows)
        _google_sheet_loaded = True
        _google_last_error = ""
        logger.info(
            "Google Sheets restored database and %s hosted files.",
            restored_files,
        )
        # If the spreadsheet was newly created or empty, keep the initialized
        # runtime database/files durable immediately instead of waiting for the
        # background loop.  This also creates the required header rows.
        sync_to_google_sheets(force=True)
        return True
    except Exception as exc:
        _google_last_error = str(exc)
        logger.error("Google Sheets initial restore failed: %s", exc)
        return False


def trigger_google_sheets_sync():
    global _google_sync_scheduled
    if not google_sheets_configured() or not _google_sheet_loaded:
        return
    if _google_sync_scheduled:
        return
    _google_sync_scheduled = True
...
def get_google_sheet_status_text():
    ...
    status = "READY" if _google_sheet_loaded and not _google_last_error else "ERROR"
    detail = f"\n\nLast error: {_google_last_error[:300]}" if _google_last_error else ""
    return (
        "📊 GOOGLE SHEETS SYNC\n\n"
        f"Status: {status}\n"
        "Tabs: DatabaseDump, HostedFiles, Summary\n"
        f"Auto sync: every {GOOGLE_SYNC_INTERVAL:g} seconds"
        f"{detail}"
    )


def mark_all_files_stopped_after_restart():
    """Restore file records safely, but never resurrect a child process."""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        conn.execute("UPDATE user_files SET is_stopped = 1")
        conn.commit()
        conn.close()
    logger.info("Startup policy applied: all hosted files are restored as stopped.")
...
def init_db():
    ...
    try:
        ...
        # auto = Binance Pay API verification; manual = Binance follows the
        # same pending/admin-approval flow as the other payment methods.
        c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('payment_mode', 'auto')")
...
# Google-only runtime persistence.  Legacy handlers continue to use the
# in-memory SQLite compatibility database, while every table and hosted file
# is serialized into Google Sheets so Render's filesystem is never durable.
def _google_runtime_snapshot_v2():
    rows = []
    with DB_LOCK:
    ...
    # Uploaded source files are part of the durable runtime snapshot too.
    # They are chunked to keep individual Sheets cells reasonably small.
    if os.path.isdir(UPLOAD_BOTS_DIR):
        for root, _, names in os.walk(UPLOAD_BOTS_DIR):
            for name in names:
                path = os.path.join(root, name)
                try:
                    relative = os.path.relpath(path, UPLOAD_BOTS_DIR)
                    encoded = base64.b64encode(
                        open(path, "rb").read()
                    ).decode("ascii")
                    for index, start in enumerate(range(0, len(encoded), 30000)):
                        rows.append({
                            "table_name": "__hosted_file__",
                            "row_index": index,
                            "row_json": json.dumps({
                    ...
                    logger.warning("Could not snapshot hosted file %s: %s", path, exc)
    return rows


def _restore_google_runtime_v2(rows):
    grouped = {}
    file_chunks = {}
    for row in rows or []:
        table = str(row.get("table_name", "")).strip()
        if table == "__hosted_file__":
            try:
                payload = json.loads(row.get("row_json", "{}"))
                file_chunks.setdefault(payload["relative_path"], []).append(
                    (int(row.get("row_index", 0)), payload.get("chunk", ""))
                )
            ...
            try:
                grouped.setdefault(table, {
                    ...
                    (int(row.get("row_index", 0)), json.loads(row.get("row_json", "{}")))
                )
    ...
    with DB_LOCK:
        conn = _sqlite_keepalive
        for table, payload in grouped.items():
            if not table.replace("_", "").isalnum():
                continue
            try:
                conn.execute(f'DELETE FROM "{table}"')
            except sqlite3.OperationalError:
                schema = payload.get("schema_sql") or ""
                if schema.upper().startswith("CREATE TABLE"):
                    conn.execute(schema)
            columns = [
                item[1] for item in conn.execute(
            ...
            for _, values in sorted(payload["rows"], key=lambda item: item[0]):
                conn.execute(
    ...
    if file_chunks:
        os.makedirs(UPLOAD_BOTS_DIR, exist_ok=True)
        for relative, chunks in file_chunks.items():
            target = os.path.abspath(os.path.join(UPLOAD_BOTS_DIR, relative))
            if os.path.commonpath([os.path.abspath(UPLOAD_BOTS_DIR), target]) != os.path.abspath(UPLOAD_BOTS_DIR):
                continue
            try:
                os.makedirs(os.path.dirname(target), exist_ok=True)
                encoded = "".join(
                    value for _, value in sorted(chunks, key=lambda item: item[0])
                )
                with open(target, "wb") as handle:
                    handle.write(base64.b64decode(encoded))
            except (OSError, ValueError) as exc:
...
def _google_rows_matrix_v2(rows, sheet_name):
    headers = GOOGLE_SHEET_HEADERS[sheet_name]
    return [headers] + [
        [row.get(header, "") for header in headers] for row in rows
    ]
...
def sync_to_google_sheets_v2(force=False):
    global _google_last_signature, _google_last_error
    if not google_sheets_configured() or not _google_sheet_loaded:
        return False
    try:
        with DB_LOCK:
            user_rows = []
            for (user_id,) in _sqlite_keepalive.execute(
                "SELECT user_id FROM active_users"
            ).fetchall():
                user_rows.append({
                    "user_id": user_id, "active": "1", "user_name": "",
                    "username": "", "balance": "", "plan": "",
                    "expiry": "", "banned": "", "user_type": "active",
                })
            hosted_rows = [{
                "owner_id": user_id, "file_name": file_name,
                "file_type": file_type,
                "desired_status": "active" if not stopped else "stopped",
                "drive_file_id": "",
            } for user_id, file_name, file_type, _, stopped in
                _sqlite_keepalive.execute(
                    "SELECT user_id,file_name,file_type,upload_time,is_stopped FROM user_files"
                ).fetchall()]
            config_rows = [{
                "setting": key, "value_json": json.dumps(value, default=str)
            } for key, value in _sqlite_keepalive.execute(
                "SELECT key,value FROM settings"
            ).fetchall()]
        runtime_rows = _google_runtime_snapshot_v2()
        rows_by_sheet = {
            "Users": user_rows,
            "BotConfig": config_rows,
            "HostedBots": hosted_rows,
            "Summary": [
                {"metric": "storage_mode", "value": "Google Sheets only; runtime SQLite is memory-only"},
                {"metric": "startup_file_policy", "value": "restart only desired_status=active"},
                {"metric": "runtime_rows", "value": len(runtime_rows)},
            ],
            "RuntimeData": runtime_rows,
        }
        ...
        base_url = f"https://sheets.googleapis.com/v4/spreadsheets/{GOOGLE_SHEET_ID}"
        _google_api(
            "POST", f"{base_url}/values:batchClear", token,
            {"ranges": [
                f"{name}!A:ZZ"
                for name in GOOGLE_SHEET_NAMES + LEGACY_GOOGLE_SHEET_NAMES
            ]},
        )
        _google_api(
            "POST", f"{base_url}/values:batchUpdate", token,
            {"valueInputOption": "RAW", "data": [
                {"range": f"{name}!A1", "values": _google_rows_matrix_v2(
                    rows_by_sheet[name], name
                )} for name in GOOGLE_SHEET_NAMES
            ]},
        )
        _google_last_signature = signature
        ...
        _google_last_error = str(exc)
        logger.error("Google-only sync failed: %s", exc)
        return False


def load_google_sheet_data_v2():
    global _google_sheet_loaded, _google_last_error
    if not google_sheets_configured():
        raise RuntimeError(
            "GOOGLE_SHEET_ID and GOOGLE_SERVICE_ACCOUNT_JSON are required; "
            "no local durable fallback is supported."
        )
    token = _google_access_token()
    if not token:
        raise RuntimeError("Could not authenticate to Google Sheets.")
    _ensure_google_sheet_tabs(token)
    rows = _google_sheet_rows(token, "RuntimeData")
    if rows:
        _restore_google_runtime_v2(rows)
    _google_sheet_loaded = True
    _google_last_error = ""
    sync_to_google_sheets_v2(force=not bool(rows))
    logger.info("Google-only runtime restore completed (%s rows).", len(rows))
    return True


def google_sheets_sync_thread_v2():
    while True:
        try:
            with _google_runtime_lock:
                sync_to_google_sheets_v2()
        except Exception as exc:
            logger.warning("Google-only background sync failed: %s", exc)
        time.sleep(GOOGLE_SYNC_INTERVAL)


def delete_all_google_data():
    """Irreversibly clear child processes, memory, uploaded files, and Sheets."""
    for key in list(bot_scripts):
        try:
            kill_process_tree(bot_scripts[key])
        except Exception:
            logger.exception("Could not stop child process %s", key)
    bot_scripts.clear()
    with DB_LOCK:
        tables = _sqlite_keepalive.execute(
            "SELECT name FROM sqlite_master WHERE type='table' "
            "AND name NOT LIKE 'sqlite_%'"
        ).fetchall()
        for (table,) in tables:
            _sqlite_keepalive.execute(f'DELETE FROM "{table}"')
        _sqlite_keepalive.commit()
    ...
    binance_plans.clear()
    for root, dirs, files in os.walk(UPLOAD_BOTS_DIR, topdown=False):
        for name in files:
            try:
        ...
        for name in dirs:
            try:
                ...
                pass
    sync_to_google_sheets_v2(force=True)
    logger.warning("ALL BOT DATA WAS DELETED by an administrator.")


# Route any legacy call sites to the Google-only implementation.
sync_to_google_sheets = sync_to_google_sheets_v2
load_google_sheet_data = load_google_sheet_data_v2
google_sheets_sync_thread = google_sheets_sync_thread_v2


# Initialize the runtime database, restore the Google-backed snapshot, then
# load the restored values into the bot's in-memory collections.
init_db()
load_google_sheet_data_v2()
load_data()
# --- End Database Setup ---
...
def get_payment_mode():
    """Return the admin-selected payment mode: auto or manual."""
    return "manual" if (get_setting("payment_mode") or "").strip().lower() == "manual" else "auto"


def is_binance_auto_payment_enabled():
    return get_payment_mode() == "auto"


def payment_mode_label():
    return (
        "AUTO — Binance Pay auto verification"
        if is_binance_auto_payment_enabled()
        else "MANUAL — Binance also needs admin approval"
    )
...
def resolve_file_callback(data, prefix):
    ...
    if len(value) == 16 and re.fullmatch(r"[0-9a-f]{16}", value):
        ...
        # The callback can outlive an in-memory refresh (or be pressed just
        # after an upload).  Resolve it from SQLite as the source of truth.
        try:
            with DB_LOCK:
                conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
                rows = conn.execute(
                    "SELECT user_id, file_name FROM user_files"
                ).fetchall()
                conn.close()
            ...
            logger.error(f"Could not resolve file callback from database: {e}")
        return None
...
def get_user_file_count(user_id):
    ...
    try:
        with DB_LOCK:
            conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
            count = conn.execute(
                "SELECT COUNT(*) FROM user_files WHERE user_id = ?", (user_id,)
            ).fetchone()[0]
            conn.close()
        return int(count)
    except Exception as e:
        logger.error(f"Could not count files for {user_id}: {e}")
        return len(user_files.get(user_id, []))


def get_user_files_from_db(user_id):
    """Return the durable file list and refresh the in-memory cache."""
    try:
        with DB_LOCK:
            conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
            rows = conn.execute(
                """SELECT file_name, file_type, is_stopped
                   FROM user_files WHERE user_id = ? ORDER BY file_name""",
                (user_id,)
            ).fetchall()
            conn.close()
        files = [(row[0], row[1]) for row in rows]
        user_files[user_id] = files
        stopped = [row[0] for row in rows if row[2]]
        if stopped:
            file_stop_status[user_id] = stopped
        else:
            file_stop_status.pop(user_id, None)
        return files
    except Exception as e:
        logger.error(f"Could not load files for {user_id}: {e}")
        return list(user_files.get(user_id, []))
...
def install_python_requirements_file(req_path, message):
    ...
    try:
        ...
        _return_to_main_menu_after_file_result(message)
        return False
    except Exception as e:
...
def _return_to_main_menu_after_file_result(message):
    """Return the uploader to the same menu shown by /start."""
    try:
        go_back_to_main(message)
    except Exception as exc:
        logger.warning("Could not restore the main menu after file result: %s", exc)
...
def run_script(
    ...
    if attempt > max_attempts:
        ...
        _return_to_main_menu_after_file_result(message_obj_for_reply)
        return
    ...
    if is_running:
        markup.row(
            types.InlineKeyboardButton(
                ...
                icon_custom_emoji_id=EMOJI_FILE_OFF,
                style="danger"
            ),
            types.InlineKeyboardButton(
                "🔄 Restart",
        ...
        markup.row(
            types.InlineKeyboardButton(
                ...
                icon_custom_emoji_id=EMOJI_FILE_ON,
                style="success"
            ),
            types.InlineKeyboardButton(
                "🗑️ Delete",
...
def handle_deposit_user(message):
    ...
    bot.reply_to(message, 
        render_body_text(
            "📌 *নিচের payment method select করুন।*"
        ),
        reply_markup=markup,
        parse_mode='HTML'
    )
...
def process_deposit_trxid(message, method_name, amount):
    ...
    # ==========================================
    if method_name.lower() == 'binance' and is_binance_auto_payment_enabled():
...
def handle_admin_deposit_panel(message):
    ...
    bot.reply_to(message, 
        render_body_text(
            ...
            f"━━━━━━━━━━━━━━━━━\n\n"
            f"⚙️ *Current payment mode:* {payment_mode_label()}\n\n"
            f"📌 𝙎𝙀𝙇𝙀𝘾𝙏 𝘼𝙉 𝙊𝙋𝙏𝙄𝙊𝙉"
        ),
        reply_markup=keyboard,
        parse_mode='HTML'
...
def handle_admin_payment_mode(message):
    """Show and change the persistent Manual/Auto payment mode."""
    if not is_otp_admin(message.from_user.id):
        bot.reply_to(message, "⛔ *Unauthorized!*", parse_mode='HTML')
        return
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
    markup.add(
        make_keyboard_button("𝘼𝙐𝙏𝙊 𝘽𝙄𝙉𝘼𝙉𝘾𝙀 𝙋𝘼𝙔", EMOJI_BINANCE, "success"),
        make_keyboard_button("𝙈𝘼𝙉𝙐𝘼𝙇 𝙋𝘼𝙔", EMOJI_DEPOSITE_METHOD, "primary"),
        make_keyboard_button("𝘽𝘼𝘾𝙆 𝙏𝙊 𝘿𝙀𝙋𝙊𝙎𝙄𝙏𝙀 𝙎𝙔𝙎𝙏𝙀𝙈", EMOJI_BACK, "danger"),
    )
    bot.reply_to(
        message,
        render_body_text(
            "⚙️ *PAYMENT MODE*\n"
            "━━━━━━━━━━━━━━━━━\n\n"
            f"📌 *Current:* {payment_mode_label()}\n\n"
            "✅ *AUTO* — Binance transaction is verified automatically.\n"
            "✅ *MANUAL* — Binance uses the normal pending request and "
            "admin approval flow."
        ),
        reply_markup=markup,
        parse_mode='HTML'
...
def set_admin_payment_mode(message, mode):
    """Persist the selected payment mode and return to the deposit panel."""
    if not is_otp_admin(message.from_user.id):
        bot.reply_to(message, "⛔ *Unauthorized!*", parse_mode='HTML')
        return
    mode = "manual" if str(mode).lower() == "manual" else "auto"
    if not set_setting("payment_mode", mode):
        bot.reply_to(message, "❌ *Payment mode save করা যায়নি!*", parse_mode='HTML')
        return
    bot.reply_to(
        message,
        render_body_text(
            f"✅ *Payment mode updated!*\n"
            f"━━━━━━━━━━━━━━━━━\n\n"
            f"⚙️ *Selected:* {payment_mode_label()}"
        ),
        parse_mode='HTML'
    )
...
def handle_otp_all_files(message):
    ...
    file_entries = []
    control_markup = types.InlineKeyboardMarkup(row_width=1)
    for i, f in enumerate(all_files_data, 1):
        is_running = is_bot_running(f["user_id"], f["file_name"])
        is_stopped = bool(f["is_stopped"])
        status_icon = file_status_emoji_tag(is_running, is_stopped)
        status_text = "ON" if is_running and not is_stopped else "OFF"
        file_entries.append(
        ...
        # ALL FILES is also the admin's management screen.  Selecting any
        # row opens the existing start/stop/restart/delete/log controls.
        if len(control_markup.keyboard) < 50:
            control_markup.add(types.InlineKeyboardButton(
                f"{f['file_name']} - {status_text}",
                callback_data=make_file_callback(
                    'stopdelete', f["user_id"], f["file_name"]
                ),
                icon_custom_emoji_id=file_status_emoji_id(is_running, is_stopped)
            ))
    control_markup.add(types.InlineKeyboardButton(
    ...
    summary = (
        ...
        f"{file_status_emoji_tag(True, False)} *Running:* {running_count}\n"
        f"{file_status_emoji_tag(False, True)} *Stopped:* {stopped_count}\n\n"
    )
    ...
    for page_number, page in enumerate(pages):
        ...
        bot_method(
            ...
            reply_markup=control_markup if page_number == 0 else None,
            parse_mode='HTML'
        )

    # Telegram keyboards are kept in manageable pages too.  The first page
    # is attached above; add controls for every remaining file so ALL FILES
    # never silently becomes "first 50 files only".
    for start in range(50, len(all_files_data), 50):
        page_markup = types.InlineKeyboardMarkup(row_width=1)
        for f in all_files_data[start:start + 50]:
            is_running = is_bot_running(f["user_id"], f["file_name"])
            is_stopped = bool(f["is_stopped"])
            status_text = "ON" if is_running and not is_stopped else "OFF"
            page_markup.add(types.InlineKeyboardButton(
                f"{f['file_name']} - {status_text}",
                callback_data=make_file_callback(
                    'stopdelete', f["user_id"], f["file_name"]
                ),
        ...
        bot.send_message(
            message.chat.id,
            render_body_text(
                f"📁 *ALL FILES — controls {start + 1}-"
                f"{min(start + 50, len(all_files_data))}*"
            ),
...
def handle_otp_stop_delete(message):
    ...
    for f in all_files_data[:50]:
        is_running = is_bot_running(f["user_id"], f["file_name"])
        is_stopped = bool(f["is_stopped"])
        status_icon = "ON" if is_running and not is_stopped else "OFF"
        btn_text = (
...
def handle_stopdelete_control(call, answer=True):
    ...
    user_files_list = get_user_files_from_db(target_user_id)
    ...
    status_text = (
        f"{file_status_emoji_tag(is_running, is_stopped)} "
        f"{'ON' if is_running and not is_stopped else 'OFF'}"
    )
...
def handle_admin_stop(call):
    ...
    if stop_user_file(target_user_id, file_name):
        ...
        try:
            bot.send_message(
                target_user_id,
                render_body_text(
                    f"{file_status_emoji_tag(False, True)} *আপনার ফাইল অ্যাডমিন দ্বারা স্টপ করা হয়েছে!*\n"
                    f"━━━━━━━━━━━━━━━━━\n\n"
                    f"📄 ফাইল: `{file_name}`\n"
                    f"👤 অ্যাডমিন: {user_id}\n\n"
                    f"⛔ *যতক্ষণ না অ্যাডমিন স্টার্ট করে ততক্ষণ নতুন ফাইল আপলোড করা যাবে না!*\n\n"
    ...
    # Re-render the same control screen so its ON/OFF state cannot become
    # stale after an admin action.
    if call.message:
        call.data = make_file_callback('stopdelete', target_user_id, file_name)
        handle_stopdelete_control(call, answer=False)
...
def handle_admin_start(call):
    ...
    try:
        bot.send_message(
            target_user_id,
            render_body_text(
                f"{file_status_emoji_tag(True, False)} *আপনার ফাইল অ্যাডমিন দ্বারা স্টার্ট করা হয়েছে!*\n"
                f"━━━━━━━━━━━━━━━━━\n\n"
                f"📄 ফাইল: `{file_name}`\n"
                f"👤 অ্যাডমিন: {user_id}\n\n"
...
def handle_admin_delete(call):
    ...
    # Remove the deleted row from the old keyboard and show a fresh
    # database-backed inventory.  This also works when the action came from
    # the ALL FILES screen rather than STOP & DELETE.
    try:
        bot.edit_message_reply_markup(
            call.message.chat.id,
            call.message.message_id,
        ...
        logger.warning(f"Could not refresh file list after deletion: {e}")
...
def go_back_to_main(message):
    ...
    elif (get_user_premium_plan(user_id)
          and get_user_premium_plan(user_id).get('expiry')
          and get_user_premium_plan(user_id)['expiry'] > datetime.now()):
        expiry_date = get_user_premium_plan(user_id)['expiry']
...
def _logic_run_all_scripts(message_or_call):
    ...
    for target_user_id, files_for_user in all_user_files_snapshot.items():
        ...
        for file_name, file_type in files_for_user:
            ...
            if is_stopped:
                status = f"{file_status_emoji_tag(False, True)} OFF"
                stopped_files += 1
            elif is_running:
                status = f"{file_status_emoji_tag(True, False)} ON"
                running_files += 1
            else:
                status = f"{file_status_emoji_tag(False, False)} OFF"
                stopped_files += 1
    ...
    summary_msg = render_body_text(
        ...
        f"{file_status_emoji_tag(True, False)} *Running:* {running_files}\n"
        f"{file_status_emoji_tag(False, True)} *Stopped:* {stopped_files}\n\n"
        f"📋 *Script Details:*\n"
        f"━━━━━━━━━━━━━━━━━\n\n"
        f"{script_list}"
    )
...
def _logic_check_files(message):
    # The admin main menu's CHECK FILES button should open the complete
    # durable inventory, not only the admin's own (usually empty) folder.
    if is_otp_admin(message.from_user.id):
        handle_otp_all_files(message)
        return

    user_id = message.from_user.id
    user_files_list = get_user_files_from_db(user_id)
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        is_stopped = user_id in file_stop_status and file_name in file_stop_status[user_id]
        status_icon = "OFF" if is_stopped or not is_running else "ON"
...
def handle_delete_all_data_request(message):
    ...
    prompt = bot.reply_to(
        message,
        render_body_text(
            "⚠️ <b>DELETE ALL BOT DATA</b>\n\n"
            "This stops every child bot, deletes all runtime records and "
            "uploaded files, and clears the Google-backed data.\n\n"
            "Type <code>DELETE ALL DATA</code> to confirm, or /cancel."
        ),
        parse_mode="HTML",
    )
    bot.register_next_step_handler(prompt, process_delete_all_data_confirmation)


def process_delete_all_data_confirmation(message):
    if not is_otp_admin(message.from_user.id):
        return
    if (message.text or "").strip() != "DELETE ALL DATA":
        bot.reply_to(message, "❌ Delete cancelled. Nothing was changed.")
        return
    try:
        delete_all_google_data()
        bot.reply_to(message, "✅ All bot data was deleted from runtime and Google.")
    except Exception as exc:
        logger.exception("Delete-all-data failed")
        bot.reply_to(message, f"❌ Delete failed: {exc}")
...
def handle_otp_deposite_submenu(message):
    ...
    if text == "𝙋𝘼𝙔𝙈𝙀𝙉𝙏 𝙈𝙊𝘿𝙀":
        handle_admin_payment_mode(message)
    elif text == "𝘼𝙐𝙏𝙊 𝘽𝙄𝙉𝘼𝙉𝘾𝙀 𝙋𝘼𝙔":
        set_admin_payment_mode(message, "auto")
    elif text == "𝙈𝘼𝙉𝙐𝘼𝙇 𝙋𝘼𝙔":
        set_admin_payment_mode(message, "manual")
    elif text == "𝘽𝘼𝘾𝙆 𝙏𝙊 𝘿𝙀𝙋𝙊𝙎𝙄𝙏𝙀 𝙎𝙔𝙎𝙏𝙀𝙈":
        handle_admin_deposit_panel(message)
    elif text == "𝙎𝙃𝙊𝙒 𝘼𝙇𝙇 𝘿𝙀𝙋𝙊𝙎𝙄𝙏𝙀 𝙍𝙀𝙌𝙐𝙀𝙎𝙏":
...
def file_control_callback(call):
    try:
        ...
        user_files_list = get_user_files_from_db(script_owner_id)
...
def start_bot_callback(call):
    try:
        ...
        status_text = (
            f"{file_status_emoji_tag(is_now_running, False)} "
            f"{'ON' if is_now_running else 'OFF (starting failed or still starting)'}"
        )
        try:
            bot.edit_message_text(
                render_body_text(f"⚙️ Controls for: `{file_name}` ({file_type}) of User `{script_owner_id}`\n{status_emoji_tag()} Status: {status_text}"),
...
def stop_bot_callback(call):
    try:
        ...
        if not is_bot_running(script_owner_id, file_name): 
            ...
            try:
                 bot.edit_message_text(
                     render_body_text(f"⚙️ Controls for: `{file_name}` ({file_type}) of User `{script_owner_id}`\n{file_status_emoji_tag(False, True)} Status: OFF"),
                     chat_id_for_reply, call.message.message_id,
...
def restore_persisted_running_files():
    """Restart only child bots whose Google-backed desired state was active."""
    logger.info("Starting recovery of previously active hosted files.")

    # Legacy recovery code is intentionally unreachable.  It remains below
    # only to make upgrades from older copies easy to compare.
    try:
...
if __name__ == '__main__':
    ...
    keep_alive()
    # Google restore completed before polling. Restart only files that were
    # active before shutdown; manually stopped files remain stopped.
    restore_persisted_running_files()
    threading.Thread(target=google_sheets_sync_thread_v2, daemon=True).start()
